...
@section('title', 'GCash Payment - ' . $order->order_number)
        ...
        <div style="margin-bottom: 30px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                    ...
                    <p style="font-size: 18px; font-weight: 700; color: #333; margin: 0;">{{ $order->order_number }}</p>
                </div>
                
                <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                    <p style="font-size: 12px; color: #666; margin: 0 0 5px 0;">Total Amount</p>
                    <p style="font-size: 18px; font-weight: 700; color: #667eea; margin: 0;">₱{{ number_format($order->total_amount, 2) }}</p>
                </div>
            </div>
            ...
            <div style="border-top: 1px solid #eee; padding-top: 20px;">
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px; font-size: 14px;">
                    <span style="color: #666;">{{ $item->artwork->title }} (Q: {{ $item->quantity }})</span>
                    <span style="font-weight: 600; color: #333;">₱{{ number_format($item->price * $item->quantity, 2) }}</span>
                </div>
                @endforeach
            </div>
        </div>
        ...
        <div style="border-top: 1px solid #eee; padding-top: 20px;">
            ...
            <div style="background: #e7f5e7; border-left: 4px solid #10b981; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                <p style="margin: 0 0 10px 0; font-size: 14px; color: #2d5e3a; line-height: 1.7;">
                    ...
                    <strong>3. Amount:</strong> ₱{{ number_format($order->total_amount, 2) }}<br>
                    <strong>4. Reference:</strong> {{ $order->order_number }}
                </p>
            </div>
            
            <form action="{{ route('payment.gcash.mock') }}" method="POST" id="gcashForm">
                @csrf
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <a href="{{ route('order.checkout', $order) }}" 
                ...
                <div style="background: white; border-radius: 12px; padding: 30px; max-width: 500px; width: 90%; 
                    ...
                    <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 25px;">
                        <div style="text-align: center;">
                            ...
                            <p style="font-size: 32px; font-weight: 700; color: #10b981; margin: 0;">₱{{ number_format($order->total_amount, 2) }}</p>
                            <p style="font-size: 13px; color: #666; margin: 10px 0 0 0;">Order: {{ $order->order_number }}</p>
                        </div>
                    </div>
                    ...
                    <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 15px; border-radius: 6px; 
                        ...
                        <p style="font-size: 14px; color: #856404; margin: 0; line-height: 1.6;">
                            ...
                            • The reference number is {{ $order->order_number }}
                        </p>
                    </div>
